...
const getMedicine = async (request, response) => {
  ...
  connection.query(query1, values1, async (err, result) => {
    ...
    const query2 =
      "SELECT medicine_id, medicine_name, description, createtime FROM medicine_master WHERE delete_flag=0 AND ( added_by = 0  OR (added_by = 1 AND user_id = ?))";
...
const getResentReports = async (request, response) => {
  ...
  connection.query(userQuery, userValues, async (err, result) => {
    ...
    try {
        ...
        LEFT JOIN report_category rc
          ON rc.report_category_id = mrm.report_category_id
...
const getBPData = async (request, response) => {
  ...
  connection.query(userQuery, userValues, async (err, result) => {
    ...
    try {
      const Query =
        "SELECT measurement_id, systolic_bp, diastolic_bp, pulse, createtime FROM measurement_master WHERE user_id = ? AND type = 0 AND delete_flag=0";
...
const getFastingGlucose = async (request, response) => {
  ...
  connection.query(userQuery, userValues, async (err, result) => {
    ...
    try {
      const Query =
        "SELECT measurement_id, fasting_glucose, createtime FROM measurement_master WHERE user_id = ? AND type = 1 AND delete_flag=0";
...
const getPPBGS = async (request, response) => {
  ...
  connection.query(userQuery, userValues, async (err, result) => {
    ...
    try {
      const Query =
        "SELECT measurement_id, ppbgs, createtime FROM measurement_master WHERE user_id = ? AND type = 2 AND delete_flag=0";
...
const getWeightMeasurement = async (request, response) => {
  ...
  connection.query(userQuery, userValues, async (err, result) => {
    ...
    try {
      const Query =
        "SELECT measurement_id, weight, createtime FROM measurement_master WHERE user_id = ? AND type = 3 AND delete_flag=0";
...
const getTemperature = async (request, response) => {
  ...
  connection.query(userQuery, userValues, async (err, result) => {
    ...
    try {
      const Query =
        "SELECT measurement_id, temperature, createtime FROM measurement_master WHERE user_id = ? AND type = 4 AND delete_flag=0";
    ...
    connection.query(dailyQuery, [user_id], (err, dailyResult) => {
      ...
      connection.query(
        ...
        (err, monthlyResult) => {
          ...
          connection.query(
            yearlyQuery,
            [user_id, currentYear - 2, currentYear + 2],
            (err, yearlyResult) => {
              if (err)
                return response.status(200).json({
                  success: false,
...
const getPPBGSDataStatus = async (request, response) => {
  ...
  const userQuery =
    "SELECT active_flag FROM user_master,delete_flag WHERE user_id = ? ";
...
const getDoctors = async (request, response) => {
  ...
  connection.query(userQuery, [user_id], (err, result) => {
    ...
    try {
      const checkDoctor =
        "SELECT doctor_id FROM patient_master WHERE user_id = ? AND delete_flag = 0";
...
const getDocumentsByReportCategory = async (request, response) => {
  ...
  try {
    ...
    connection.query(checkUser, [user_id], async (err, res) => {
      ...
      const sql =
        "SELECT report_category_id FROM medical_report_master WHERE user_id =? AND delete_flag =0";
      connection.query(sql, [user_id], async (err1, res1) => {
        ...
        let report_category_ids = [
          ...new Set(res1.map((item) => item.report_category_id)),
        ];
        let document_arr = [];
        for (const categoryId of report_category_ids) {
          let document_count = await getDocumentCountByCategory(
            categoryId,
            user_id,
          );
...
async function getDocumentCountByCategory(categoryId, user_id) {
  return new Promise((resolve, reject) => {
    const sql =
      "SELECT COUNT(medical_report_id) AS total_count FROM medical_report_master WHERE report_category_id = ? AND user_id = ? AND delete_flag = 0";
...
async function getReportCategoryName(categoryId) {
  return new Promise((resolve, reject) => {
    const sql =
      "SELECT category_name FROM report_category WHERE report_category_id  = ? AND delete_flag = 0";
  ...
  return new Promise((resolve, reject) => {
    ...
    connection.query(sql, [user_id, category_id], (err, results) => {
      ...
      if (!results || results.length === 0) {
        return resolve(0.0); // No files → 0 MB
      }
  ...
  try {
    ...
    connection.query(sql, [user_id], async (err, res) => {
      ...
      if (res.length == 0) {
        return response
          .status(200)
          .json({ success: false, msg: request.__("data_not_found") });
      }
      ...
      const sql =
        "SELECT doctor_id, user_id FROM patient_master WHERE user_id = ? AND doctor_id = ? AND delete_flag = 0";
      connection.query(sql, [user_id, doctor_id], async (err1, res1) => {
        ...
        if (res1.length > 0) {
          const update =
            "UPDATE patient_master SET delete_flag = 1, updatetime = NOW() WHERE user_id = ? AND doctor_id = ?";
          connection.query(update, [user_id, doctor_id], async (err2, res2) => {
            ...
            if (res2.affectedRows > 0) {
              return response.status(200).json({
                success: true,
                msg: request.__("doctor_deleted_successfully"),
              });
            } else {
              return response.status(200).json({
                success: false,
                msg: request.__("doctor_has_not_been_deleted_please_try_again"),
              });
            }
          });
        } else {
  ...
  try {
    ...
    if (type == 0) {
      ...
      if (!serviceResult.length)
        return res
          .status(200)
          .json({ success: false, msg: "No service found" });
      ...
      const cityPriceResult = await new Promise((resolve, reject) => {
        ...
        connection.query(sql, [service_id, city], (err, result) =>
          err ? reject(err) : resolve(result),
        );
      });
      const cityPrice = cityPriceResult.length
        ? cityPriceResult[0].price
        : service.price;
      ...
      const avgRating = totalReviews
        ? (
            reviewsResult.reduce((sum, r) => sum + r.rating, 0) / totalReviews
          ).toFixed(1)
        : "0";
      ...
      const cartResult = await new Promise((resolve, reject) => {
        ...
        connection.query(sqlCartMaster, [user_id], (err, carts) => {
          ...
          connection.query(sqlCartItem, [cart_id, service_id], (err, items) => {
            if (err) return reject(err);
            if (!items.length)
              return resolve({ service_count: 0, cart_flag: 0 });
            return resolve({
              service_count: items[0].service_count,
              cart_flag: 1,
            });
          });
        });
      ...
      const categoryResult = await new Promise((resolve, reject) => {
        ...
        connection.query(sql, [service.category_id], (err, result) =>
          err ? reject(err) : resolve(result),
        );
      });
      const categoryName = categoryResult.length
    ...
    if (type == 1) {
      ...
      if (!packageResult.length)
        return res
          .status(200)
          .json({ success: false, msg: "No package found" });
      const pkg = packageResult[0];

      // 4️⃣ Get city price
      const cityPriceResult = await new Promise((resolve, reject) => {
        const sql = `SELECT price FROM service_price_master WHERE service_id = ? AND city_name = ? AND delete_flag = 0`;
        connection.query(sql, [package_id, city], (err, result) =>
          err ? reject(err) : resolve(result),
        );
      });
      const cityPrice = cityPriceResult.length
      ...
      return res
        .status(200)
        .json({ success: true, msg: languageMessage.dataFound, packages });
    }
  } catch (error) {
...
const shareInfoToDoctor = async (req, res) => {
  ...
  try {
    ...
    connection.query(userSql, [user_id], (err, user) => {
      ...
      connection.query(insertSql, [user_id, doctor_id, infoTypeStr], (err2) => {
        if (err2) {
          return res.status(200).json({
            success: false,
            msg: req.__("internal_server_error"),
...
const deleteMeasurement = async (req, res) => {
  try {
    const { measurement_id, user_id, language_code } = req.body;
    ...
    connection.query(checkUserQuery, [user_id], (err, userResult) => {
      ...
      if (userResult.length === 0) {
        return res
          .status(200)
          .json({ success: false, msg: req.__("user_not_found") });
      }
      ...
      const updateQuery =
        "UPDATE measurement_master SET delete_flag = 1 WHERE measurement_id = ? AND user_id = ? AND delete_flag = 0";
      connection.query(
        updateQuery,
        ...
        (err1, updateResult) => {
          ...
          return res.status(200).json({
            success: true,
            msg: req.__("data_deleted_successfully"),
          });
        },
      );
    });
