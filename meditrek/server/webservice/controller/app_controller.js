...
const createtime = moment()
  .tz(process.env.TIME_ZONE || "Europe/Paris")
  ...
  try {
    ...
    connection.query(query1, values1, async (err, result) => {
      ...
      connection.query(newUserQuery, values, async (err, result) => {
        ...
        return response.status(200).json({
          success: true,
          msg: request.__("thank_you_for_contacting_us"),
...
const pauseMedication = async (request, response) => {
  ...
  connection.query(query1, values1, async (err, result) => {
    ...
    const query2 =
      "UPDATE medication_master SET pause_status = ? WHERE medication_id = ? AND delete_flag = 0";
    ...
    connection.query(query2, values2, async (err, result) => {
      ...
      return response
        .status(200)
        .json({ success: true, msg: languageMessage.medicationPaused });
    });
  });
};
...
const MedicationMarkASTaken = async (request, response) => {
  ...
  connection.query(query1, values1, async (err, result) => {
    ...
    connection.query(getMedicationQuery, [time_slots_id], (err, medResult) => {
      ...
      const averagesql =
        "INSERT INTO medicine_average_master (status, medicine_id, user_id, time_slots_id, taken_datetime, createtime,updatetime) VALUES(?, ?, ?, ?, ?, now(),now())";
      connection.query(
        averagesql,
          ...
          connection.query(
            ...
            (err, updateResult) => {
              ...
              const updateMedicationQuery =
                "UPDATE medication_master SET remaining_quantity = remaining_quantity - dosage WHERE medication_id = ? AND delete_flag = 0 AND remaining_quantity > 0";
              connection.query(
...
const editNewMedication = async (request, response) => {
  ...
  connection.query(userQuery, userValues, async (err, result) => {
    ...
    const checksql =
      "SELECT medication_id  ,user_id ,medicine_id FROM medication_master WHERE user_id = ? AND medication_id = ? AND delete_flag = 0";
    connection.query(checksql, [user_id, medication_id], async (err, check) => {
      ...
      connection.query(insertQuery, insertValues, async (err, insertResult) => {
        ...
        if (remainder_time && typeof remainder_time === "string") {
          ...
          if (timeSlots.length > 0) {
            const timeSlotValues = timeSlots.map((slot) => [
              medicationId,
              slot,
            ]);
            ...
            const updatesql =
              "UPDATE time_slots_master SET delete_flag = 1 ,updatetime = ? WHERE medication_id = ? ";
            connection.query(
              updatesql,
...
const DeleteMedication = async (request, response) => {
  ...
  const userQuery =
    "SELECT mobile, active_flag,delete_flag FROM user_master WHERE user_id = ? ";
...
const addMedicalReport = async (request, response) => {
  const { user_id, report_category_id, document_size, language_code } =
    request.body;
  ...
  connection.query(userQuery, userValues, async (err, result) => {
    ...
    const insertValues = [user_id, report_category_id, file, document_size];
    ...
    connection.query(insertQuery, insertValues, async (err, insertResult) => {
      ...
      return response.status(200).json({
        ...
        msg: request.__("report_added_successfully"),
...
const addBPData = async (request, response) => {
  const { user_id, systolic_bp, diastolic_bp, pulse, language_code } =
    request.body;
  ...
  connection.query(userQuery, [user_id], (err, result) => {
    ...
    connection.query(
      ...
      (err) => {
        ...
        return response
          .status(200)
          .json({ success: true, msg: request.__("bp_added_successfully") });
      },
    );
  });
};
...
const addPPBGS = async (request, response) => {
  ...
  connection.query(userQuery, [user_id], (err, result) => {
    ...
    connection.query(insertQuery, [user_id, ppbgs], (err) => {
      ...
      return response
        .status(200)
        .json({ success: true, msg: request.__("ppbgs_added_successfully") });
    });
  });
};
...
const addCustomMeasure = async (request, response) => {
  ...
  connection.query(userQuery, [user_id], (err, result) => {
    ...
    connection.query(insertQuery, [user_id, symptom, symptom_range], (err) => {
      ...
      return response
        .status(200)
        .json({ success: true, msg: request.__("data_inserted_successfully") });
    });
  });
};
...
const editCustomMeasure = async (request, response) => {
  const { user_id, measurement_id, symptom, symptom_range, language_code } =
    request.body;
        ...
        if (timeSlots.length > 0) {
          ...
          connection.query(
            ...
            (err, timeSlotResult) => {
              ...
              return response.status(200).json({
                success: true,
                msg: request.__("medication_created_successfully"),
                insertId: medicationId,
              });
            },
          );
    ...
    connection.query(checkUser, [user_id], async (err, res) => {
      ...
      connection.query(checkDoctor, [doctor_id], async (err, res) => {
        ...
        if (res.length == 0) {
          return response
            .status(200)
            .json({ success: false, msg: request.__("doctor_not_found") });
        }

        const insert =
          "INSERT INTO report_share_master(user_id, medical_report_id, doctor_id, share_type, createtime, updatetime) VALUES(?, ?, ?, 1, ?, ?)";
        connection.query(
          insert,
...
async function getUserMedicineAdded(user_id) {
  return new Promise((resolve, reject) => {
    const sql =
      "SELECT user_id, medication_id FROM medication_master WHERE user_id = ? AND delete_flag = 0";
...
async function getUserReportAdded(user_id) {
  return new Promise((resolve, reject) => {
    const sql =
      "SELECT user_id, medical_report_id FROM medical_report_master WHERE user_id = ? AND delete_flag = 0";
    connection.query(sql, [user_id], (err, res) => {
      if (err) {
        reject(err);
      } else {
...
async function getMeasurementAdded(user_id) {
  return new Promise((resolve, reject) => {
    const sql =
      "SELECT user_id, measurement_id FROM measurement_master WHERE user_id = ? AND delete_flag = 0";
    connection.query(sql, [user_id], (err, res) => {
      if (err) {
        reject(err);
      } else {
...
async function getAdverseReactionAdded(user_id) {
  return new Promise((resolve, reject) => {
    const sql =
      "SELECT user_id, adverse_reaction_id FROM adverse_reaction_master WHERE user_id = ? AND delete_flag = 0";
    connection.query(sql, [user_id], (err, res) => {
      if (err) {
        reject(err);
      } else {
...
const DeleteMedicationFromHistory = async (request, response) => {
  ...
  connection.query(userQuery, userValues, async (err, result) => {
    ...
    const check =
      "SELECT user_id, delete_flag FROM medication_master WHERE user_id = ? AND medication_id = ?";
    connection.query(
      check,
  ...
  try {
    ...
    connection.query(check, [user_id], async (checkErr, checkRes) => {
      ...
      connection.query(update, [user_id], async (updateErr, updateRes) => {
        ...
        return response
          .status(200)
          .json({ success: true, msg: languageMessage.playerIdRemoved });
      });
    });
  } catch (error) {
    return response.status(200).json({
...
const AddMeasurementReminder = async (request, response) => {
  const {
    user_id,
    measurement_type,
    schedule,
  ...
  connection.query(userQuery, [user_id], async (err, result) => {
    ...
    if (
      result[0]?.active_flag === 0 ||
      result[0]?.delete_flag == 1
    ) {
      return response.status(200).json({
        success: false,
        msg: request.__("user_deactivated"),
...
const DeleteMeasurementReminder = async (request, response) => {
  const {
    user_id,
    measurement_reminder_id,
    language_code,
  ...
  if (!user_id || !measurement_reminder_id) {
    return response.status(200).json({
      success: false,
      msg: languageMessage.msg_empty_param,
    });
    ...
    connection.query(
      ...
      async (err, deleteResult) => {
        ...
        return response.status(200).json({
          success: true,
          msg: request.__(
            "measurement_reminder_deleted_successfully"
          ),
        });
      }
    );
