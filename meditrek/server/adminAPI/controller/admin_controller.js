...
const { getUserLanguage } = require("../../webservice/helpers/languageHelper");
...
const getAllSymptoms = async (request, response) => {
  try {
    ...
    // MAIN QUERY (UNCHANGED FLOW, only EN via translation like FAQ)
    const getsymptomsql = `
      SELECT 
        sm.symptom_id,
    ...
    connection.query(getsymptomsql, (err, results) => {
      ...
      //  GET ALL TRANSLATIONS 
      const translationSql = `
        SELECT symptom_id, language_code, symptom_name, description
        FROM symptoms_translation
      `;
      ...
      connection.query(translationSql, (err, translations) => {
        ...
        translations.forEach((t) => {
          if (!translationMap[t.symptom_id]) {
            translationMap[t.symptom_id] = {};
          }

          translationMap[t.symptom_id][t.language_code] = {
            symptom_name: t.symptom_name,
            description: t.description,
          };
        });

        // FINAL ARRAY (NO CHANGE, only translations added)
        let s_no = 0;

        const symptom_arr = results.map((symptom) => {
          s_no++;

          return {
            s_no: s_no,
            symptom_id: symptom.symptom_id,
            symptom_name: symptom.symptom_name || "N/A",
            description: symptom.description || "N/A",
            createtime: symptom.createtime,

            //  ONLY NEW FIELD (same as FAQ)
            translations: translationMap[symptom.symptom_id] || {},
          };
        });
...
const editSymptom = async (request, response) => {
  try {
    const { symptom_id, symptom_name, translations } = request.body;
    ...
    // Check duplicate (same as before)
    const checkSymptomSql =
      "SELECT symptom_id FROM symptoms_master WHERE symptom_name = ? AND symptom_id != ? AND delete_flag = 0";
      ...
      (err, results) => {
        ...
        connection.query(
          ...
          (err) => {
            ...
            //  NEW: UPSERT translations (same as FAQ)
            const tasks = Object.entries(translations).map(([lang, data]) => {
              return new Promise((resolve, reject) => {

                const checkLangSql = `
                  SELECT id FROM symptoms_translation 
                  WHERE symptom_id = ? AND language_code = ?
                `;

                connection.query(checkLangSql, [symptom_id, lang], (err, result) => {
                  if (err) return reject(err);

                  if (result.length > 0) {
                    // 🔁 UPDATE
                    const updateLangSql = `
                      UPDATE symptoms_translation 
                      SET symptom_name = ?, description = ?
                      WHERE symptom_id = ? AND language_code = ?
                    `;

                    connection.query(
                      updateLangSql,
                      [data.symptom_name, data.description || null, symptom_id, lang],
                      (err) => {
                        if (err) return reject(err);
                        resolve();
                      }
                    ...
                    connection.query(
                      insertLangSql,
                      [symptom_id, lang, data.symptom_name, data.description || null],
                      (err) => {
                        if (err) return reject(err);
                        resolve();
                      }
            ...
            Promise.all(tasks)
              .then(() => {
                ...
                response.status(200).json({
                  success: true,
                  msg: languageMessages.symptomUpdated,
                });
              })
              .catch((error) => {
                return response.status(200).json({
...
// };
const getAdminPatientDemographics = (req, res) => {
  const { doctor_ids, gender, age_group } = req.body;
  ...
  if (hasDoctors) {
    ...
    matchWhere += ` AND pm.doctor_id IN (${ph})`;
    baseParams.push(...doctor_ids);
    matchParams.push(...doctor_ids);
  }
  // When no doctors selected, we DON'T add any doctor filter
  // This will include ALL patients who have at least one entry in patient_master
  // (which means they're linked to at least one doctor)
  ...
  // Total patients (patients linked to selected doctors OR all patients if no doctor selected)
  const totalSql = `
    SELECT COUNT(DISTINCT pm.user_id) AS total
    FROM patient_master pm
  ...
  connection.query(totalSql, baseParams, (err, totalResult) => {
    ...
    connection.query(dataSql, matchParams, (err2, rows) => {
      ...
      rows.forEach(r => {
        if (ageSexCross[r.age_group]) {
          ageSexCross[r.age_group][r.gender] = r.count;
          ageSexCross[r.age_group].total += r.count;
          ageDist[r.age_group] += r.count;
        }
        if (sexDist[r.gender] !== undefined) {
          sexDist[r.gender] += r.count;
        }
      });
      ...
      const crossTable = ageGroups
        ...
        .map(a => ({
          age_group: a,
          ...genders.reduce((acc, g) => {
            acc[g] = `${ageSexCross[a][g]} (${total_patients > 0
                ? ((ageSexCross[a][g] / total_patients) * 100).toFixed(1)
                : "0.0"
              }%)`;
            return acc;
          }, {}),
          total: `${ageSexCross[a].total} (${total_patients > 0
...
const getPatientDemographicsDetailsAdmin = (req, res) => {
  const { doctor_ids, gender, age_group, search, page = 1, limit = 10 } = req.body;
  ...
  // If no doctors selected, include ALL patients who have ANY doctor link
  ...
  if (age_group && age_group !== "") {
    if (age_group.includes("+")) {
      matchWhere += ` AND TIMESTAMPDIFF(YEAR, um.dob, CURDATE()) >= ?`;
      matchParams.push(parseInt(age_group.replace("+", ""), 10));
  ...
  // 1. Total patients (distinct patients linked to selected doctors)
  const totalSql = `
    SELECT COUNT(DISTINCT pm.user_id) AS total
    FROM patient_master pm
    INNER JOIN user_master um ON pm.user_id = um.user_id
  ...
  connection.query(totalSql, totalParams, (err, countResult) => {
    if (err) return res.json({ success: false, msg: "Count error", error: err.message });

    const total = countResult[0]?.total || 0;

    // 2. Matched patients count (with all filters)
    const matchSql = `
    ...
    connection.query(matchSql, matchParams, (err2, matchResult) => {
      if (err2) return res.json({ success: false, msg: "Match error", error: err2.message });

      const matched_patients = matchResult[0]?.matched || 0;

      // 3. Paginated rows - Get DISTINCT patients with their details
      // We need to get one record per patient, even if they have multiple doctors
      const dataSql = `
        SELECT DISTINCT
          pm.user_id,
          um.name,
        ...
        INNER JOIN user_master um ON pm.user_id = um.user_id
        ${matchWhere}
        GROUP BY pm.user_id, um.name, um.dob, um.gender, um.diseases
        ORDER BY um.name ASC
        LIMIT ? OFFSET ?
      `;

      connection.query(dataSql, [...matchParams, Number(limit), Number(offset)], (err3, rows) => {
        if (err3) return res.json({ success: false, msg: "Data error", error: err3.message });
        ...
        const promises = rows.map(patient => new Promise((resolve, reject) => {
          ...
          const medSql = `
            SELECT DISTINCT a.medicine_id, a.medicine_name
            FROM medication_master m
            INNER JOIN medicine_master a ON a.medicine_id = m.medicine_id
            INNER JOIN time_slots_master tm ON tm.medication_id = m.medication_id
            WHERE m.user_id = ?
              AND m.delete_flag = 0
              AND tm.delete_flag = 0
            ORDER BY a.medicine_name ASC
          ...
          connection.query(medSql, [patient.user_id], (err5, meds) => {
...
const getDiseaseDashboardAdmin = (req, res) => {
  ...
  if (hasDoctors) {
    ...
    where += ` AND p.doctor_id IN (${placeholders})`;
  ...
  connection.query(totalSql, hasDoctors ? doctor_ids : [], (err, totalRes) => {
    ...
    connection.query(matchedSql, params, (err, matchRes) => {
      ...
      connection.query(diseaseCountSql, params, (err, countRes) => {
        if (err) return res.json({ success: false, msg: err.message });
        ...
        const diseaseSql = `
          ...
          ${where}
          GROUP BY u.diseases
          ORDER BY count DESC, u.diseases ASC
          LIMIT ? OFFSET ?
        `;

        connection.query(
          diseaseSql,
          [...params, Number(disease_limit), Number(diseaseOffset)],
          (err, diseaseRows) => {
            if (err) return res.json({ success: false, msg: err.message });
            ...
            connection.query(ageSql, params, (err, ageRows) => {
              ...
              const sexSql = `
                ...
                ${where}
                GROUP BY u.gender
                ORDER BY FIELD(gender, 'Male', 'Female', 'Other', 'Not Specified')
              `;
              ...
              connection.query(sexSql, params, (err, sexRows) => {
                ...
                const patientSql = `
                  ...
                  JOIN user_master u ON u.user_id = p.user_id
                  ${where}
                  GROUP BY u.user_id, u.name, u.dob, u.gender, u.diseases
                  ORDER BY u.name ASC
                  LIMIT ? OFFSET ?
                `;
                    ...
                    const promises = patientRows.map(
                      (patient) =>
                        new Promise((resolve, reject) => {
                          ...
                          let shareParams = [patient.user_id];

                          if (hasDoctors) {
                            checkShare += ` AND doctor_id IN (${doctor_ids.map(() => "?").join(", ")})`;
                            shareParams.push(...doctor_ids);
                          }

                          checkShare += ` ORDER BY createtime DESC`;
                          ...
                          connection.query(
                            ...
                            (err1, shareList) => {
                              ...
                              const latestShare = shareList.find((r) =>
                                r.information_type && r.information_type.split(",").includes("1"),
                              );
                    ...
                    Promise.all(promises)
                      .then((finalPatients) => {
                        return res.json({
                          ...
                          disease_total: totalDiseaseGroups,
                          age_breakdown,
                          sex_breakdown,
                          patients: finalPatients,
                        });
...
const getPatientDiseasesMedicineDashboardAdmin = (req, res) => {
  ...
  const filterByMedications = (patients, medicationList) => {
  ...
  let absoluteTotalSql = `
    ...
    WHERE p.delete_flag = 0
      ...
      AND u.dob <= CURDATE()
  `;
  let absoluteTotalParams = [];

  if (hasDoctors) {
    const placeholders = doctor_ids.map(() => "?").join(", ");
    absoluteTotalSql += ` AND p.doctor_id IN (${placeholders})`;
    absoluteTotalParams.push(...doctor_ids);
  }
  ...
  connection.query(absoluteTotalSql, absoluteTotalParams, (err, absoluteTotalResult) => {
    if (err) return res.json({ success: false, msg: err.message });

    const totalPatients = absoluteTotalResult[0].total;
    ...
    connection.query(dataSql, params, (err, patients) => {
      ...
      connection.query(medSql, userIds, (err, medications) => {
        ...
        let filtered = filterByMedications(allPatients, medication);
        ...
        if (Array.isArray(disease) && disease.length > 0) {
          ...
          if (singleOnly && disease.length === 1) {
            ...
            filtered = filtered.filter(p => {
              const dis = (p.diseases || []).map(d => d.toLowerCase().trim());
              return dis.length === selected.length && selected.every(d => dis.includes(d));
            });
          } else if (includeExtra && disease.length >= 2) {
            filtered = filtered.filter(p => {
              const dis = (p.diseases || []).map(d => d.toLowerCase().trim());
        ...
        const medOffset = (Number(med_page) - 1) * Number(med_limit);
        const paginatedMedicineSummary = medicineSummary.slice(
          medOffset,
          medOffset + Number(med_limit)
        );

        // ── Therapy Distribution (combo per patient) ──
        const therapyMap = {};
        unique.forEach(p => {
          const combo = p.medications.length > 0
            ? [...p.medications.map(m => m.medicine_name)].sort().join(", ")
            : "No Medication";
        ...
        const allTherapy = Object.entries(therapyMap)
          ...
          .map(([combo, count]) => ({
            ...
            patient_count: count,
            percent_matched: matchedPatients
              ? ((count / matchedPatients) * 100).toFixed(2)
              : "0.00",
            percent_total: totalPatients
              ? ((count / totalPatients) * 100).toFixed(2)
              : "0.00",
          }));

        const therapyOffset = (Number(therapy_page) - 1) * Number(therapy_limit);
        const paginatedTherapy = allTherapy.slice(
          therapyOffset,
          therapyOffset + Number(therapy_limit)
        );

        // ── Drilldown patients ──
        const drilldownOffset = (Number(drilldown_page) - 1) * Number(drilldown_limit);
        const paginatedDrilldown = unique.slice(
          drilldownOffset,
          drilldownOffset + Number(drilldown_limit)
        );
        ...
        return res.json({
          ...
          analytics: {
            ...
            percentage: totalPatients > 0
              ? ((matchedPatients / totalPatients) * 100).toFixed(2) + "%"
              : "0%",
            top_drug: topDrug,
            doctors_count: hasDoctors ? doctor_ids.length : 0,
          },
          ...
          patients: {
            data: paginatedDrilldown,
            total: matchedPatients,
            page: Number(drilldown_page),
            limit: Number(drilldown_limit),
            total_pages: Math.ceil(matchedPatients / drilldown_limit),
          }
        });
      });
    });
...
const getAdminMedicationFull = (req, res) => {
  let {
    ...
    summary_page = 1,
    summary_limit = 10,
    therapy_page = 1,      // ← added
    therapy_limit = 10,    // ← added
    patient_page = 1,
    patient_limit = 10,
  } = req.body;
  ...
  const therapyOffset = (Number(therapy_page) - 1) * Number(therapy_limit);  // ← added
    ...
    connection.query(patientSql, params, (err, patients) => {
      ...
      if (patients.length === 0) {
        return res.json({
          ...
          summary: [],
          summary_total: 0,
          therapy_distribution: [],   // ← added
          therapy_total: 0,           // ← added
          details: [],
          patient_total: 0,
          graph: [],
          drilldown: [],
      ...
      Promise.all(promises).then((allPatients) => {
        ...
        const uniqueMap = {};
        filteredPatients.forEach((p) => {
          if (!uniqueMap[p.user_id]) uniqueMap[p.user_id] = p;
        });
        ...
        // ── Age Breakdown ──
        const ageGroups = { "0-18": 0, "19-30": 0, "31-44": 0, "45-64": 0, "65-74": 0, "75-84": 0, "85+": 0 };
        ...
        const age_breakdown = Object.keys(ageGroups).map((key) => ({
          age_group: key,
          count: ageGroups[key],
          percentage: matchedCount ? ((ageGroups[key] / matchedCount) * 100).toFixed(2) : "0.00",
        }));

        // ── Sex Breakdown ──
        const genderGroups = { Male: 0, Female: 0, Other: 0, "Not Specified": 0 };
        uniquePatients.forEach((p) => {
          const g = p.gender || "Not Specified";
          if (genderGroups[g] !== undefined) genderGroups[g]++;
          else genderGroups[g] = 1;
        });
        const sex_breakdown = Object.keys(genderGroups).map((key) => ({
          gender: key,
          count: genderGroups[key],
          percentage: matchedCount ? ((genderGroups[key] / matchedCount) * 100).toFixed(2) : "0.00",
        }));
        ...
        const allDrugSummary = Object.keys(medMap)
          .map((name) => ({
            medicine_name: name,
            patient_count: medMap[name],
            percentage: matchedCount
              ? ((medMap[name] / matchedCount) * 100).toFixed(2) + "%"
              : "0.00%",
          }))
          .sort((a, b) => b.patient_count - a.patient_count);

        // ── Therapy Distribution (combos per patient) ──  ← new block
        const therapyMap = {};
        uniquePatients.forEach((p) => {
          const meds = (p.medications || [])
  ...
  if (symptoms.length > 0) {
    where += ` AND EXISTS (
      ...
      WHERE r2.user_id = u.user_id
        ${doctor_ids.length > 0 ? `AND r2.doctor_id IN (${doctor_ids.map(() => "?").join(",")})` : ""}
        AND r2.share_type = 0
  ...
  connection.query(totalSql, baseParams, (err, totalRes) => {
    ...
    const dataSql = `
      SELECT 
        ...
        (
          SELECT JSON_ARRAYAGG(
            DISTINCT JSON_OBJECT('id', a.medicine_id, 'name', a.medicine_name)
          )
          FROM report_share_master r
          JOIN medication_master m 
            ON m.user_id = r.user_id 
          ...
          JOIN time_slots_master tm 
            ...
            AND tm.delete_flag = 0
          WHERE r.user_id = u.user_id
            ${doctor_ids.length > 0 ? `AND r.doctor_id IN (${doctor_ids.map(() => "?").join(",")})` : ""}
            AND r.share_type = 0
            AND r.delete_flag = 0
    ...
    connection.query(dataSql, [...extraParams, ...params], (err2, users) => {
      ...
      let matchedPatients = finalPatients.filter((p) => p.user_id !== null);
      ...
      if (Array.isArray(disease) && disease.length > 0) {
        ...
        if (singleOnly && disease.length === 1) {
          ...
          matchedPatients = matchedPatients.filter((p) => {
            const dis = (p.diseases || []).map((d) => d.toLowerCase().trim());
            if (includeExtra) return selectedDiseases.every((d) => dis.includes(d));
            return dis.length === selectedDiseases.length && selectedDiseases.every((d) => dis.includes(d));
          });
        } else {
          matchedPatients = matchedPatients.filter((p) =>
            (p.diseases || []).some((d) => selectedDiseases.includes(d.toLowerCase().trim()))
          );
        }
      }
      ...
      if (Array.isArray(medication) && medication.length > 0) {
        ...
        if (singleOnly && medication.length === 1) {
          ...
          matchedPatients = matchedPatients.filter((p) => {
            const meds = (p.medications || []).map((m) => m?.name?.toLowerCase().trim()).filter(Boolean);
            if (includeExtra) return selectedMeds.every((m) => meds.includes(m));
      ...
      if (Array.isArray(symptoms) && symptoms.length > 0) {
        ...
        if (singleOnly && symptoms.length === 1) {
          ...
          matchedPatients = matchedPatients.filter((p) => {
            const syms = (p.reported_symptoms || []).map((s) => s.toLowerCase().trim());
            if (includeExtra) return selectedSymptoms.every((s) => syms.includes(s));
            return syms.length === selectedSymptoms.length && selectedSymptoms.every((s) => syms.includes(s));
          });
        } else {
          matchedPatients = matchedPatients.filter((p) =>
            (p.reported_symptoms || []).some((s) => selectedSymptoms.includes(s.toLowerCase().trim()))
          );
        }
      }
      ...
      // ── Age Distribution ──
      const ageGroups = { "0-18": 0, "19-30": 0, "31-44": 0, "45-64": 0, "85+": 0 };
      ...
      const age_distribution = Object.entries(ageGroups)
        .filter(([, count]) => count > 0)
        .map(([age_group, count]) => ({
          age_group,
          count,
          percentage: matched > 0 ? parseFloat(((count / matched) * 100).toFixed(1)) : 0,
        }));
      ...
      matchedPatients.forEach((p) => {
        const g = p.gender || "Not Specified";
        genderGroups[g] = (genderGroups[g] || 0) + 1;
      });
      const gender_distribution = Object.entries(genderGroups).map(([gender, count]) => ({
        gender,
        count,
        percentage: matched > 0 ? parseFloat(((count / matched) * 100).toFixed(1)) : 0,
      }));
      ...
      const disease_distribution = Object.entries(diseaseCounts)
        .sort((a, b) => b[1] - a[1])
        .map(([disease, count]) => ({
          disease,
          count,
      ...
      const medication_distribution = Object.entries(medCounts)
        .sort((a, b) => b[1] - a[1])
      ...
      const symptom_distribution = Object.entries(symptomCounts)
        .sort((a, b) => b[1] - a[1])
        .map(([symptom, count]) => ({
          symptom,
          count,
